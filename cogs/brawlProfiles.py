...
class brawlProfiles(commands.Cog):
  ...
  async def save_id(self, interaction: discord.Interaction, bs_id: str):
    ...
    player_name, bs_id = getPlayerNameForId(bs_id)
    if not player_name:
      await interaction.edit_original_response(generalTexts["invalidId"][language], ephemeral=True, delete_after=3)
    else:
      user_options = mongodb.findUserOptions(interaction.user.id)
      user_options["bs_id"] = bs_id 
      await interaction.edit_original_response(generalTexts["invalidId"][language].format(bs_id=bs_id), ephemeral=True, delete_after=3)
